...
\paragraph{Self-improvement machinery
(\texttt{no\_self\_improvement}, \texttt{no\_retroverify},
\texttt{no\_l2\_anchor}, \texttt{no\_abort\_guard}).}
\texttt{no\_self\_improvement} freezes weights at $\theta_0$ and is
the strictest test of the self-improvement loop's cumulative
contribution; we predict a large negative $\Delta\text{CES}$ (at least
$-0.05$) driven primarily by the ACC axis, with the per-cycle CES
trace flat after Cycle~$0$ rather than rising, because every later
cycle's improvement in the reference run is by construction
attributable to training. Falsification condition: if
...
exhibiting a visibly shallower slope than the reference.
\texttt{no\_l2\_anchor} (setting $\lambda = 0$) should produce a
moderate negative RET axis effect ($-0.03$ to $-0.06$) as the MMLU
retention ratio drifts downward cycle-over-cycle without the anchor
pulling weights back toward $\theta_{\text{prev}}$; ACC on the
in-domain training benchmarks may be neutral or slightly positive
(unconstrained weights can fit the training pool harder), which is
precisely why the CES composite's multiplicative structure matters.
\texttt{no\_abort\_guard} disables the $\rho < 0.93$ forgetting
circuit-breaker and should produce a catastrophic RET-axis collapse
($-0.10$ or worse) on a minority of seeds where the training pool
happens to carry a distribution shift, with the guard's absence being
the only thing separating a recoverable cycle from an unrecoverable
one. Because this effect is rare in expectation, a single-seed
Phase~1 run may under-detect it; Phase~2's independent-seeds report
is the correct place to confirm this prediction.

\paragraph{Memory and routing
(\texttt{no\_tier1}, \texttt{no\_retrieval}, \texttt{no\_routing},
\texttt{no\_cold\_start}, \texttt{no\_memory\_prune}).}
\texttt{no\_tier1} forces every query through Tier~2 or Tier~3 and
should produce a moderate negative $\Delta\text{CES}$ ($-0.02$ to
$-0.04$) driven jointly by ACC (memory-answerable queries now route
to noisier tiers) and latency (not a CES axis but reported in
Table~\ref{tab:headline}); the effect should grow over cycles as the
reference memory accumulates more hits. \texttt{no\_retrieval}
disables Tier~3 RAG entirely and should produce a large negative EPI
and grounding-axis effect on benchmarks that rely on out-of-parametric
knowledge (primarily TriviaQA and Natural Questions), with a
smaller effect on benchmarks that are answerable from the base
model's own parametric memory (FEVER label accuracy, StrategyQA).
The unsupported-correct rate in Table~\ref{tab:grounding} should
\emph{rise} under \texttt{no\_retrieval}, not fall, because the
remaining correct answers are now entirely parametric and carry no
retrieval support by definition. \texttt{no\_routing} disables the
OR-condition safety veto and routes purely on the pre-confidence
$u_{\text{pre}}$; we predict a moderate negative EPI and VER effect
($-0.02$ to $-0.04$) concentrated on confident-but-wrong cases, with
the pooled confident-error rate in Table~\ref{tab:halluc} rising
visibly. \texttt{no\_cold\_start} begins Cycle~0 with an empty memory
and should produce a moderate negative $\Delta\text{CES}$ at Cycles~0
and~1 that narrows substantially by Cycle~10 as the memory populates
organically; the primary diagnostic is the per-cycle CES trace rather
than the final-cycle scalar, and the cycle at which the variant
catches up to the reference run measures the cold-start's marginal
value. \texttt{no\_memory\_prune} should produce a small positive or
neutral $\Delta\text{CES}$ on the benchmark panel combined with an
unbounded memory size growth and a rising retrieval latency; this is
the ablation whose primary cost is system-level rather than
accuracy-level, and its ``result'' is the memory-growth curve in
Table~\ref{tab:memory-growth} rather than a CES delta.

\paragraph{Verification signals
(\texttt{no\_verification}, \texttt{no\_nli}, \texttt{no\_selfcons},
\texttt{no\_entropy}, \texttt{no\_mcdropout}).}
\texttt{no\_verification} accepts every generation into memory
regardless of $u_{\text{stored}}$ and is the hardest test of the
verifier's pool-purification role. We predict a large negative
$\Delta\text{CES}$ ($-0.06$ or worse) with the EPI and VER axes
collapsing and the $P_{\text{obs}}$ column in
Table~\ref{tab:purity} falling toward $p$ rather than rising above
it; this is the direct empirical mirror of
Theorem~\ref{thm:purity}'s prediction that purification requires
$\alpha > \tfrac{1}{2}$ and that a chance-equivalent verifier
contributes no purification. \texttt{no\_nli}, \texttt{no\_selfcons},
\texttt{no\_entropy}, and \texttt{no\_mcdropout} each drop a single
signal from the nine-signal composite; the verifier balanced
accuracy $\alpha$ drops in proportion to each signal's marginal
information contribution, and the resulting $\Delta\text{CES}$ band
should be $-0.01$ to $-0.03$ per signal. We expect \texttt{no\_nli}
and \texttt{no\_entropy} to have the largest individual effects
because NLI is the dominant grounding-check signal and semantic
entropy is the Farquhar-style hallucination detector the literature
claims is the highest single-signal predictor; if \texttt{no\_mcdropout}
shows a larger $\Delta\text{CES}$ than either, the $K = 5$ dropout
budget may be doing more work than the literature suggests for
Flan-T5-scale models, which would be a finding worth reporting
rather than hiding.

\paragraph{Threshold sensitivity (\texttt{lower\_u\_threshold}).}
\texttt{lower\_u\_threshold} replaces the $u_{\text{stored}} \geq 0.50$
admission cut with $u_{\text{stored}} \geq 0.30$, admitting roughly
twice as many episodes per cycle into the training pool. The expected
effect is a trade: ACC may rise slightly at early cycles (the
training pool is larger and covers more of the distribution), but EPI
falls because a lower threshold admits more confident-but-wrong
episodes, and RET may degrade because the training pool now contains
more near-miss labels. The multiplicative CES composite should trend
negative ($-0.01$ to $-0.03$) despite the ACC-axis ambiguity, which
is the precise motivation for having set the default threshold at
$0.50$ rather than at a lower value; if \texttt{lower\_u\_threshold}
instead produces a \emph{positive} $\Delta\text{CES}$, the default
threshold is miscalibrated and the Chapter~6 revision agenda should
revisit it.
...
predictions are inverted (for instance, \texttt{no\_retrieval}
...
\texttt{no\_verification} by less than $0.05$ in $\Delta\text{CES}$,
