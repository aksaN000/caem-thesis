...
percent. The quantiles are fit on the pooled 500-sample Cycle 0
evaluation set (the same calibration fold that fits $T^{*}$) and
held fixed for cycles one through N so that the decision tree
operates on stationary thresholds even as the verifier backend,
the base-model weights, or the composite weights shift between
cycles. An assertion in the calibration script enforces
