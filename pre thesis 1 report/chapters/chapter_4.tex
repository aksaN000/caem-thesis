...
$s_{\text{avg}}$ & Mean unique-pair ($i<j$) chain cosine similarity (Layer~2) \\
...
% ---- memory store ----
\node[mem]        (mem) at (5.2, -4.0)  {Memory\\$K\!=\!1\text{M}$};
...
\noindent\textbf{Tier~3 retrieval corpus.}
Tier~3 RAG retrieval uses the full DPR Wikipedia passage corpus \cite{karpukhin-etal-2020-dense},
consisting of 21,015,324 passages derived from the December 2018 Wikipedia dump segmented
into non-overlapping 100-word chunks.
Passages are indexed with a FAISS IVF-PQ index (65,536 coarse centroids, $M=64$ bytes per
vector, 8 bits per sub-quantiser), occupying approximately 40\,GB on disk and requiring at
least 128\,GB system RAM for full in-memory operation on a cloud A100 node.
At query time, the top-$k$ passages (default $k=5$) are concatenated with the query and
passed to the Flan-T5 encoder as additional context.
This corpus provides broad factual grounding for queries where neither episodic memory nor
standalone model generation is sufficiently reliable.
...
near-miss retrieval, which is particularly important for FEVER, where adversarially
...
The mean cosine similarity over unique chain pairs ($i<j$), $s_{\text{avg}}$, measures agreement across chains.
...
Semantic entropy $H_{\text{SE}}$ is computed by unconditionally generating $K=10$ diverse 
candidate answers at elevated temperature ($T=1.0$) \cite{farquhar2024semantic}. 
These candidates are grouped into semantic equivalence classes using the NLI model. 
Specifically, agglomerative clustering is applied where two answers are placed in the same 
cluster $G_c$ if and only if they demonstrate bidirectional entailment. 
The probability of each cluster is defined proportionally, $p_c = |G_c| / K$, allowing 
entropy to be computed across distinct meanings as $H_{\text{SE}} = -\strut\smash{\sum_c p_c \log p_c}$.
...
Tier~2 and Tier~3 generation uses task-aware prompts that produce structured reasoning
traces for all three training benchmarks (FEVER, TriviaQA, Natural Questions).
For classification benchmarks (FEVER), the generated output has the form
\mbox{``\texttt{Reasoning: <explanation>}''} followed by \mbox{``\texttt{Answer: <label>}''}.
At evaluation time, a dedicated extraction function (\texttt{extract\_fever\_label()})
uses regular-expression matching to parse the label
...
$s_{\text{avg}}$ is the mean SC cosine similarity over unique chain pairs ($i<j$), and
...
\Statex \textbf{Layer 1: NLI Entailment}
...
\Statex \textbf{Layer 2: Self-Consistency}
\StatePar{$s_{\text{avg}} \leftarrow$ mean cosine similarity over unique pairs of $\{c_j\}$ ($i<j$) \Comment{reuse chains from Layer~1}}
\Statex \textbf{Layer 3: Semantic Entropy}
...
\texttt{s\_avg} & float & Mean unique-pair ($i<j$) SC cosine similarity \\
...
violating the clean generate/improve separation that the verified rejection sampling
framework requires.
...
When memory approaches capacity ($K = 1{,}000{,}000$ episodes), pruning triggers
at $95\%$ occupancy (950,000 episodes).
...
At the chosen scale, the FAISS IVF-PQ index (4,096 coarse centroids, $M\!=\!64$,
8 bits per sub-quantiser) occupies approximately 70\,MB in memory, well within GPU
VRAM alongside both language models.
...
The ten-cycle structure is illustrated in Figure~\ref{fig:self-improvement-cycles}.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    cyc/.style={rectangle, draw=#1!70!black, fill=#1!8, line width=1.4pt,
                text width=2.6cm, align=center, minimum height=1.4cm,
                rounded corners=5pt, font=\small\bfseries},
    dots/.style={rectangle, draw=none, fill=none,
                 text width=1.0cm, align=center, font=\Large\bfseries},
    act/.style={rectangle, draw=gray!50, fill=gray!5, line width=0.9pt,
                text width=2.4cm, align=center, minimum height=0.9cm,
                rounded corners=3pt, font=\scriptsize},
    arr/.style={->, >=stealth, line width=1.4pt, #1},
    lbl/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
]
%% Cycle nodes (horizontal layout: C0, C1, C2, ..., C9)
\node[cyc=teal]   (c0) at (0.0,  0) {\textbf{Cycle~0}\\$\theta_0$ base\\calibration};
\node[cyc=orange] (c1) at (3.2,  0) {\textbf{Cycle~1}\\$\theta_1$\\retroverify};
\node[cyc=orange] (c2) at (6.4,  0) {\textbf{Cycle~2}\\$\theta_2$\\retroverify};
\node[dots]       (el) at (9.2,  0) {$\cdots$};
\node[cyc=orange] (c8) at (12.0, 0) {\textbf{Cycle~8}\\$\theta_8$\\retroverify};
\node[cyc=green]  (c9) at (15.2, 0) {\textbf{Cycle~9}\\$\theta_9$ final\\(inference)};

%% Main flow arrows
\draw[arr=teal!70]   (c0) -- node[above, lbl] {verified\\eps.} (c1);
\draw[arr=orange!70] (c1) -- node[above, lbl] {verified\\eps.} (c2);
\draw[arr=orange!70] (c2) -- (el);
\draw[arr=orange!70] (el) -- (c8);
\draw[arr=orange!70] (c8) -- node[above, lbl] {verified\\eps.} (c9);

%% Per-cycle activity annotations below
\node[act] (cal)  at (0.0,  -2.6) {Calibration\\(once, after C0)};
\node[act] (proc) at (5.6,  -2.6) {Query processing\\+ Stage~8 FT\\$\mathcal{L}_{\text{CE}}+L_2$};
\node[act] (rver) at (11.0, -2.6) {Retroactive\\re-verification\\(all cycles)};

\draw[->, >=stealth, dashed, gray!50, line width=0.8pt] (cal)  -- (c0.south);
\draw[->, >=stealth, dashed, gray!50, line width=0.8pt] (proc) -- (c1.south);
\draw[->, >=stealth, dashed, gray!50, line width=0.8pt] (proc) -- (c2.south);
\draw[->, >=stealth, dashed, gray!50, line width=0.8pt] (rver) -- (c8.south);
\draw[->, >=stealth, dashed, gray!50, line width=0.8pt] (rver) -- (c9.south);

%% Purity arrow spanning full width (above)
\draw[->, >=stealth, line width=1.8pt, purple!70]
    ([yshift=8pt]c0.north) -- ([yshift=8pt]c9.north)
    node[midway, above, font=\scriptsize\bfseries, text=purple!70]
        {$P_n$ increases monotonically (Thm.~\ref{thm:monotone})};

\end{tikzpicture}
}%
\caption{CAEM ten-cycle self-improvement structure (C0--C9). Calibration of
$\uhat$ signal weights runs once after Cycle~0 (Section~\ref{sec:calibration}).
Each subsequent cycle fine-tunes on freshly verified episodes and runs retroactive
re-verification to refresh memory quality scores.
Memory purity $P_n$ increases monotonically across all cycles
(Theorem~\ref{thm:monotone}, Section~\ref{sec:monotonicity}).
The final weights $\theta_9$ are used for inference after Cycle~9.}
...
All three training benchmarks use structured generation prompts that ensure
\texttt{reasoning\_chain} always contains genuine reasoning content:
\begin{itemize}
  ...
  \item \textbf{Open-ended} (TriviaQA):
        \begin{quote}\ttfamily Question: \{q\}\\ Think step by step:\\ Answer:\end{quote}
  \item \textbf{Open-ended} (Natural Questions):
        \begin{quote}\ttfamily Question: \{q\}\\ Think step by step:\\ Answer:\end{quote}
\end{itemize}
Transfer evaluation benchmarks (TruthfulQA, StrategyQA, ARC-Challenge) use the same
open-ended or classification templates respectively, but their samples are never used
for self-improvement training; they appear only in held-out evaluation.
...
comparing $\Delta_{\text{avg}}$ across successive cycle pairs.
\end{remark}
