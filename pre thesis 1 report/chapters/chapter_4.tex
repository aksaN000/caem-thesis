...
CAEM composes three pretrained models that are treated as fixed assets for the duration of an experimental run. Only Flan-T5-Large receives gradient updates in the self-improvement loop described in Section~\ref{sec:self-improvement}; the sentence encoder and the NLI cross-encoder are frozen throughout. Table~\ref{tab:pretrained-components} summarises the three checkpoints, their parameter counts, and their role in the pipeline.
...
NLI cross-encoder & RoBERTa-Large-MNLI \citep{liu2019roberta} & $355$M & Entailment and contradiction probabilities for $s_{\mathrm{avg}}$, $p_{\mathrm{entail}}$, $p_{\mathrm{ground}}$, and $p_{\mathrm{contra}}$; frozen. \\
\bottomrule
\end{tabular}
\end{table}
