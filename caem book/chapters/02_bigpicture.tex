...
\begin{tikzpicture}[
  ...
  % outer frame
  \node[outer, minimum width=13.2cm, minimum height=5.6cm] (O) at (0,0) {};
  ...
  \node[inner, minimum width=8.4cm, minimum height=2.5cm] (I) at (-2.0,0.45) {};
  \node[anchor=north west, font=\footnotesize\bfseries\sffamily,
        text=cbIntuition!80!black] at (I.north west)
        {\,\,PER-QUERY CLOCK \;(Stages 1--7): ticks thousands of times};
  \node[align=center, font=\footnotesize, text=cbInk!85] at (-2.0,0.2)
  ...
  \node[cbstep, text width=3.0cm] (sil)  at (4.0, 1.55) {Fine-tune on verified\\answers + retention guard};
  \node[cbstep, text width=3.0cm] (recal)at (4.0, 0.45) {Recalibrate verifier\\+ retro-verify memory};
  \node[cbstep, text width=3.0cm] (grow) at (4.0,-0.65) {Grow memory with\\fresh-data stream};

  % flows
  \draw[flow] (I.east) -- ++(0.5,0) |- (sil.west);
  \draw[flow] (sil) -- (recal);
  \draw[flow] (recal) -- (grow);
  \draw[carry] (grow.south) |- ($(I.south)+(0,-0.55)$)
        node[pos=0.75, below, font=\scriptsize, text=cbExample!60!black]
...
\begin{tikzpicture}[
  ...
  io/.style={rectangle, rounded corners=2pt, draw=cbInk!55, fill=cbInk!6,
    line width=1pt, align=center, inner sep=5pt, minimum height=0.9cm},
  front/.style={rectangle, rounded corners=4pt, draw=cbIntuition!75!black,
    ...
    minimum height=1.0cm, drop shadow={shadow xshift=0.3ex,
  ...
  tier/.style={rectangle, rounded corners=4pt, draw=cbScenario!70!black,
    ...
    minimum height=0.85cm, text width=2.3cm, drop shadow={shadow xshift=0.3ex,
    shadow yshift=-0.3ex, fill=black!10}},
  ...
  \draw[bypass] (t1.east) -| (out.west)
    node[pos=0.25, above, font=\scriptsize, text=cbScenario!70!black]
    {no verify, no store};
  ...
  \draw[store] (gate.north) -- ++(0,0.6) -| (store.south)
    node[pos=0.25, right, font=\scriptsize, text=cbExample!55!black]
    {store / defer};
\end{tikzpicture}}
...
\begin{tikzpicture}[
  ...
  \draw[no] (guard) -- node[above,font=\scriptsize,text=cbPitfall!70!black]
    {fail} (revert);
  \draw[flow] (keep) -| (recal);
  \draw[flow] (recal) -- (grow);
  \draw[flow] (revert) |- (grow);
\end{tikzpicture}
