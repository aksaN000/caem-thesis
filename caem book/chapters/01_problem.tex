...
\begin{tikzpicture}[
  ...
  \node[flaw=cbIntuition] (state) at (90:3.8) {\textbf{Stateless}\\[2pt]
    {\footnotesize no verified answer is retained}};
  \node[flaw=cbPitfall]   (conf)  at (210:3.8) {\textbf{Overconfident}\\[2pt]
    {\footnotesize cannot tell right from wrong}};
  \node[flaw=cbExample]   (static)at (330:3.8) {\textbf{Static}\\[2pt]
  ...
  \draw[rel] (state) to[bend left=15]
    node[midway, above left=2mm, text width=2.5cm, align=right, font=\scriptsize]
    {no memory, so nothing to calibrate about} (conf);
  \draw[rel] (conf) to[bend left=15]
    node[midway, below=2mm, text width=2.8cm, align=center, font=\scriptsize]
    {cannot spot gaps, so cannot target learning} (static);
  \draw[rel] (static) to[bend left=15]
    node[midway, above right=2mm, text width=2.5cm, align=left, font=\scriptsize]
