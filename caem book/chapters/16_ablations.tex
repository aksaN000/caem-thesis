...
\begin{tikzpicture}[
  ...
  \node[claim] (c4) at (0,-1.5) {no catastrophic\\[2pt]forgetting};
  \node[metric] (m1) at (5.6,3.0) {per-tier shares + accuracy\\[3pt](\Cref{ch:trajectory}, measured)};
  \node[metric] (m2) at (5.6,1.8) {purity validation\\[3pt](\Cref{ch:theory}, measured)};
  \node[lesion] (l1) at (5.6,0.6) {no-retroverify\\[3pt](lesion, deferred)};
  \node[metric] (m3) at (5.6,-0.6) {cold-start vs trained\\[3pt](\Cref{ch:benchmarks}, measured)};
  \node[lesion] (l2) at (5.6,-1.8) {no-forgetting-guard\\[3pt](observed at C4)};
  \draw[flow] (c1) -- (m1);
  \draw[flow] (c2) -- (m2);
  \draw[flow] (c2) -- (l1);
  \draw[flow] (c3) -- (m3);
