...
\begin{tikzpicture}[
  ...
  card/.style={rectangle, rounded corners=5pt, line width=1.2pt, align=left,
    inner sep=7pt, text width=6.0cm, minimum height=2.3cm,
  ...
  \node[card, draw=cbExample!70!black, fill=cbExample!6] (c1) at (0,0) {
    ...
    \hfill {\color{cbInk!60}\footnotesize high $\cdot$ 70\%}\\[3pt]
    {\footnotesize\color{cbInk!70}\itshape Roquefort Cheese is made from what sort of milk?}\\[2pt]
    {\large sheep's milk.}};
  % DEFERRED / provisional
  \node[card, draw=cbScenario!70!black, fill=cbScenario!6] (c2) at (7.2,0) {
    {\large\color{cbScenario!55!black}$\sim$}\;\textbf{\color{cbScenario!50!black}provisional}
    \hfill {\color{cbInk!60}\footnotesize moderate $\cdot$ 55\%}\\[3pt]
    {\footnotesize\color{cbInk!70}\itshape What was President Gerald Ford's middle name?}\\[2pt]
    {\large Rudolph.}\\[2pt]
    {\footnotesize\color{cbInk!60} I'll reconsider this at my next learning cycle. Please ask again later for an updated answer.}};
  % DISCARD / conflicting
  \node[card, draw=cbNumbers!75!black, fill=cbNumbers!6] (c3) at (0,-3.0) {
    {\large\color{cbNumbers!70!black}$!$}\;\textbf{\color{cbNumbers!65!black}conflicting}
    \hfill {\color{cbInk!60}\footnotesize low $\cdot$ 40\%}\\[3pt]
    {\large\color{cbInk!45} (answer withheld)}\\[2pt]
    {\footnotesize\color{cbInk!60} Evidence exists but some signals disagree, independent verification is recommended.}};
  % ABSTAIN / insufficient
  \node[card, draw=cbPitfall!70!black, fill=cbPitfall!6] (c4) at (7.2,-3.0) {
    {\large\color{cbPitfall!65!black}$\times$}\;\textbf{\color{cbPitfall!60!black}insufficient}
    \hfill {\color{cbInk!60}\footnotesize very low}\\[3pt]
    {\large I do not know.}\\[2pt]
    {\footnotesize\color{cbInk!60} no supporting evidence was found.}};
\end{tikzpicture}
