...
\begin{tikzpicture}[
  ...
  \node[th] (bayes) at (0,1.8) {\textbf{purity identity}\\{\footnotesize $\alpha>1/2 \Rightarrow \pi>p$}};
  \node[th] (floor) at (4.2,1.8) {\textbf{purity floor}\\{\footnotesize $\pi_{t+1}\ge\min(\cdot,\cdot)$}};
  \node[th] (mono) at (0,-0.6) {\textbf{monotonicity}\\{\footnotesize better gen.\ stores more}};
  \node[th] (conv) at (4.2,-0.6) {\textbf{convergence}\\{\footnotesize bounded band}};
  \node[cor] (tier1) at (8.6,1.8) {\textbf{Tier-1 floor}\\{\footnotesize $\mathrm{CHM}_1\le 1-\pi$}};
  \node[cor] (self) at (8.6,0.0) {\textbf{self-correction}\\{\footnotesize errors pruned\\[-1pt]in $\le 1/\delta$}};
  \node[cor] (eq) at (8.6,-1.8) {\textbf{stochastic equilibrium}\\{\footnotesize commit-paced,\\[-1pt]memory grows}};
