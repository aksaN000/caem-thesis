...
\begin{tikzpicture}[
  ...
  \node[fin] (gate) at (-3.2,0) {gate says\\\textbf{store}};
  ...
  \draw[flow] (live.north) to[bend left=22] (fb.east);
  \draw[flow] (fb.west) to[bend left=22] (live.north west);
  ...
  \draw[gone] (retro) -- node[pos=0.55,above,font=\scriptsize,text=cbPitfall!70!black]{below floor} (rm);
  \draw[flow] (cons) -- (rep);
  \draw[gone] (cons) -- node[pos=0.5,above,font=\scriptsize,text=cbPitfall!70!black]{non-rep} (rm);
  \draw[gone] (prune) -- (rm);
\end{tikzpicture}
