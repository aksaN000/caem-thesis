...
Third, the conformal precision contract holds against the same-fold protocol. The conformal procedure requires exchangeability between the calibration fold and the held-out evaluation fold, so that the calibration-fold empirical-precision lower bound transfers to the evaluation distribution under the standard split-conformal argument. The protocol does \emph{not} require exchangeability between cycle $N$'s calibration fold and cycle $N+1$'s calibration fold; the same-fold choice across cycles is therefore not a contract violation.

Fourth, exponential-moving-average smoothing on the conformal thresholds (factor $\alpha_{\text{ema}} = 0.7$) absorbs single-cycle fitting noise. With the same fold every cycle, any noise that creeps in is attenuated by 70~percent under EMA over each cycle boundary, so the trajectory of $\tau_{\text{store}}$ and $\tau_{\text{defer}}$ across cycles is dominated by genuine verifier-distribution drift rather than fold-resampling variance.
...
Three points reconcile the two modes. First, the fitting procedure is fold-agnostic. The per-signal isotonic regression is a deterministic monotone fit on whatever (signal, label) pairs are supplied; sklearn's isotonic regression does not care whether the questions were the same as in a previous fit or different. The conformal-threshold fit walks the labelled fold sorted by composite and picks the threshold at the precision target, again fold-agnostic. Both procedures work identically on a fixed fold (research) or a rotating fold (production).
