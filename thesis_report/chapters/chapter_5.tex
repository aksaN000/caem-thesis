...
% reports the trajectory of headline metrics across the ten-cycle
% measurement horizon. Section 5.2 reports the calibration discipline
...
The pooled headline metrics across the ten-cycle measurement horizon are auto-generated by \texttt{scripts/make\_tables.py} from the per-cycle aggregation outputs. The headline pooled table lands at \texttt{figures/auto/tab\_headline.tex} once the main run completes; the per-cycle progression table at \texttt{figures/auto/tab\_cycle\_progression.tex} reports the same metrics broken out by cycle.
...
The geometric envelope of \Cref{thm:convergence} predicts $\mathbb{E}[G_t] \le (1-r)^t G_0$ with $r = \mathcal{O}(\sigma \cdot \Phi(d/\sqrt{2}))$ on the committed-cycle subsequence; the realistic-policy refinement of \Cref{cor:stochastic-equilibrium} replaces this envelope on the calendar timeline with $\mathbb{E}[G_t] \le (1 - \pi_{\mathrm{commit}} \cdot r)^t G_0$, where the empirical commit fraction $\pi_{\mathrm{commit}}$ is read from the cycle-by-cycle retention-guard firing pattern. The fixed-point statement of \Cref{thm:bayes-convergence} predicts that the recurrence $p_{c+1} = f(p_c)$ converges to $p^* = 1$ asymptotically on the committed-cycle subsequence. The empirical receipt fits the geometric decay to the per-cycle gap trajectory on both timelines (committed-cycle and calendar) and reports the realised half-life against the predicted range of $1.3$ to $2.2$ cycles from \Cref{sec:env-cycles}; under realised commit fractions strictly below one, the calendar-timeline half-life is correspondingly longer by a factor of $1/\pi_{\mathrm{commit}}$. The fixed-point claim is asymptotic and is not directly provable in ten cycles: the implementation has fine-tuning regularisation that pins the effective ceiling strictly below one, so the report includes the fitted ceiling $p_\infty^{\text{fit}}$ as a measurement of the implementation's effective limit. A non-geometric trajectory on the committed-cycle subsequence or a fitted ceiling far below the predicted regime would falsify the contraction-rate lower bound or the within-class Gaussian assumption; a degenerate commit fraction ($\pi_{\mathrm{commit}} \in \{0, 1\}$ across the entire trajectory) or a memory store that stalls on rolled-back cycles would falsify \Cref{cor:stochastic-equilibrium}'s independence claim; all diagnostics are reported alongside the trajectory.
...
The EWC fine-tuning baseline trains for the same realised cycle horizon as CAEM with an $L_2$ adapter anchor as the registered continual-learning mechanism, alongside an MMLU-only retention probe and rollback guard, but strips the nine-signal verifier, the calibrated probability composite, the fixed-threshold storage gate, the episodic memory, the routing, and the multi-modal probe extension that CAEM uses. The single-probe retention guard on B7 is a deliberate scope choice for the baseline panel rather than an oversight: the EWC family in the published literature uses single-probe guards, so the matched-protocol comparison registers B7 at the published EWC configuration rather than at a CAEM-extended variant, and the contrast between B7 and CAEM accordingly isolates the verification-and-memory machinery plus the multi-modal extension as a single contribution rather than two. B7's $L_2$ anchor is the elastic-weight-consolidation alternative to CAEM's bounded low-rank-adapter envelope: on the shipped CAEM configuration the anchor coefficient is set to zero because the bounded adapter envelope plus the multi-modal probe panel together carry the retention contract, while B7 activates the anchor as a separate registered continual-learning mechanism to compare against. B7 isolates the contribution of the verification-and-memory machinery from the contribution of the retention machinery. The contrast between B6 and B7 measures the retention-machinery effect on the same training signal; the contrast between B7 and CAEM measures the verification-and-memory effect on top of a continual-learning baseline that uses a different retention mechanism. Together B6 and B7 decompose CAEM's training-time contribution into two orthogonal axes.
...
The architectural ablation panel removes one mechanism at a time from the locked configuration and measures the per-cycle effect on the headline metrics. The panel is deliberately narrow: it runs only on those mechanisms whose contribution cannot be read off a directly-measured metric on the full-CAEM trajectory itself. The three-tier dispatch share, the calibration-fold empirical-precision anchor, the per-cycle pool-purity audit, the per-signal correlation matrix, and the natural-language-inference judge interchangeability test each license a single architectural claim through a metric that the full-CAEM trajectory already reports, and an additional ablation that removes the same mechanism would produce a redundant reading. The panel therefore restricts to the three mechanisms whose claim cannot be answered by a directly-measured metric: the cycle-boundary retroactive re-verification pass, the self-improvement fine-tune itself, and the multi-modal-probe rollback guard. Each ablation has a pre-registered effect direction and a pre-registered effect band, recorded before the panel runs so that within-band and out-of-band outcomes can be flagged transparently. \Cref{tab:ablation-bands} summarises the panel; the ablation registry is enumerated in \texttt{caem/ablation/variants.py}.
...
\midrule
A1 No retroverify       & Cycle-boundary retroactive re-verification pass over the stored pool & negative (purity down) & $-2$ to $-5$ pp on per-cycle pool purity \\
A2 No self-improvement  & Per-cycle low-rank-adapter fine-tune; cycle-zero pristine model retained at every cycle & negative (accuracy stagnates, tier share frozen) & $-2$ to $-6$ pp on pooled exact match at the cycle horizon, with Tier 2 share frozen at cycle zero \\
A3 No retention guard   & Multi-modal-probe rollback that aborts a cycle when worst per-probe retention falls below the registered floor & negative (retention down) & $-3$ to $-7$ pp on the worst-probe retention ratio at the cycle horizon \\
\bottomrule
\end{tabular}
\caption{Architectural ablation panel: pre-registered expected-effect direction and band per ablation. The three ablations are the lesions registered in \texttt{caem/ablation/variants.py}, each tied to a specific numbered claim whose evidence cannot be supplied by a direct trajectory metric. Each band is registered before the panel runs; realised effects within the band are reported as confirming the mechanism, out-of-band effects are flagged in \Cref{sec:comp-falsifications}.}
...
H4 & The worst per-probe retention ratio over the multi-modal probe panel stays above the registered floor under the bounded low-rank-adapter envelope and the rollback guard. & Retention trajectory in \Cref{sec:retention} & \textit{main-run pending} \\
H5 & The cycle-boundary mechanisms registered in \texttt{caem/ablation/variants.py} (retroactive re-verification, per-cycle self-improvement fine-tune, multi-modal-probe rollback guard) each contribute non-trivially to the headline outcome. & Architectural ablation panel in \Cref{sec:comp-ablations} & \textit{ablation-run pending} \\
\bottomrule
\end{tabular}
...
\Cref{sec:hypotheses} (H4) & Worst per-probe retention above floor under bounded adapter envelope + rollback guard & \Cref{sec:retention} \\
...
\Cref{thm:convergence} & Convergence envelope (committed-cycle subsequence)  & \Cref{sec:check-convergence} \\
