...
\draw[arrow=green]  (route.east) to[out=20, in=180] (t1.west);
\draw[arrow=orange] (route) -- (ruc);
\draw[arrow=red]    (ruc.east) to[out=20, in=180]  (t2.west);
\draw[arrow=red]    (ruc.east) to[out=-20, in=180] (t3.west);

%% Safety override (pre-route low confidence -> RUC, which then decides T2 vs T3)
\draw[darrow=red]   (pre.south) to[out=-90, in=180, looseness=1.4] node[elabel, pos=0.7] {safety} (ruc.west);
...
\caption[Architecture overview]{Eight-stage pipeline: pre-route, encode, route, Stage 3b retrieval-utility classifier on the score-formula and safety-override fall-through to the retrieval-augmented tier, tier execution, verify, decide, output, and a cycle-boundary self-improvement block; full specification in \Cref{ch:methodology}.}
...
\end{tikzpicture}
\caption[Experimental workflow]{Self-improvement trajectory shown schematically as early, mid, and late phases; six cycles realised under the ten-cycle registered cap and closed under the equilibrium-saturation gate at the cycle-five close. Each phase runs the per-query pipeline (Stages 1 to 7, including the Stage 3b retrieval-utility classifier that refines the retrieval-augmented dispatch into either the zero-shot tier or the retrieval-augmented tier on a per-query basis) with nine-signal post-generation verification and the four-outcome storage decision over a fresh workload. At each cycle boundary (Stage 8) a retroactive re-verification pass refreshes stored composites under the updated generator, a deferred-entry reconsideration pass sweeps the deferred buffer, a memory consolidation pass collapses near-duplicate episodes, low-rank-adaptation fine-tuning draws from a strict training pool through a benchmark-balancing reweighting layer, and a multi-modal retention probe panel spanning a general-knowledge multiple-choice surface, an open-text factoid surface, and a commonsense multiple-choice surface aborts the cycle with an adapter rollback when the worst probe ratio falls below a fixed floor. Numeric values of the retention floor, the training threshold, the storage cut-point, and related thresholds are specified in \Cref{ch:methodology}. The memory is never rolled back, so accumulated knowledge survives abort events. Headline results across the realised six-cycle measurement horizon, including the composite hallucination metric reduction against the eight-baseline panel, the per-cycle trajectory, and the retrieval-utility-classifier on/off ablation, are reported in \Cref{ch:evaluation}.}
