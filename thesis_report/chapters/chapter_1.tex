...
\end{tikzpicture}
\caption[Experimental workflow]{Ten-cycle self-improvement trajectory, shown schematically as early, mid, and late phases. Each phase runs the per-query pipeline (Stages 1 to 7) with nine-signal post-generation verification and the four-outcome storage decision over a fresh workload. At each cycle boundary (Stage 8) a retroactive re-verification pass refreshes stored composites under the updated generator, a deferred-entry reconsideration pass sweeps the deferred buffer, a memory consolidation pass collapses near-duplicate episodes, low-rank-adaptation fine-tuning draws from a strict training pool through a benchmark-balancing reweighting layer, and a multi-modal retention probe panel spanning a general-knowledge multiple-choice surface, an open-text factoid surface, and a commonsense multiple-choice surface aborts the cycle with an adapter rollback when the worst probe ratio falls below a fixed floor. Numeric values of the retention floor, the training threshold, the storage cut-point, and related thresholds are specified in \Cref{ch:methodology}. The memory is never rolled back, so accumulated knowledge survives abort events. Headline results across the eleven measurement cycles, including the composite hallucination metric reduction against the baseline panel, the per-cycle trajectory, and the ablation grid, are reported in \Cref{ch:evaluation}.}
