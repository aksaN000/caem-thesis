...
\caption{%
The eight-stage CAEM pipeline. Stages~S1--S8 are described in \Cref{ch:methodology}; the retrieval-utility classifier (Stage S3b, RUC) of \Cref{sec:ruc} sits between the three-tier router and the retrieval-augmented tier, intercepting the safety-veto and score-formula fall-through dispatches and redirecting retrieval-hurt queries to the zero-shot tier. The nine deployed signals at S5 are the registered set minus $h_{\text{norm}}$ (registered then retired at the cycle-zero audit; see the list-of-symbols footnote) and $p_{\text{contra}}$ (structurally zero under the deployed MiniCheck judge). The outer dashed arrow indicates the cycle-boundary handoff from the close of one cycle to the start of the next.}
