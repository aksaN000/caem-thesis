% Standalone wrapper for the CAEM system-architecture diagram.
% Source: thesis_report/figures/system_architecture_figure.tex
%
% This wrapper extracts the tikzpicture body verbatim and stubs the two
% \Cref calls that the thesis caption uses, so the diagram self-contains
% when compiled outside the main thesis document. The figure environment
% and caption are intentionally dropped: the poster supplies its own
% caption via the poster source.

\documentclass[tikz,border=4pt]{standalone}

% --- TikZ libraries used by the source figure ----------------------------
\usepackage{tikz}
\usetikzlibrary{arrows.meta}
\usetikzlibrary{positioning}
\usetikzlibrary{shapes.geometric}
\usetikzlibrary{fit}
\usetikzlibrary{calc}     % required by the ($(s8.north)+(-2.5,0)$) coordinate arithmetic

% --- Math support for the inline formulae in the node labels ------------
\usepackage{amsmath}
\usepackage{amssymb}

% --- Stubs for the thesis cross-references --------------------------------
% In the thesis these resolve via cleveref to "Chapter 4" and to the
% retrieval-utility classifier section. Outside the thesis they are
% rendered as plain italic text so the figure compiles standalone.
\providecommand{\Cref}[1]{\textit{the methodology chapter}}
\providecommand{\cref}[1]{\textit{the methodology chapter}}
...
\node[simp] (s8) at (6, -27.5) {%
    ...
    (2)~Curate training pool ($\tau_{\text{train}}{=}0.70$;\;pool-reweighting layer)\quad
...
\draw[rarr]
    (ruc.south) --
    node[elab,right,pos=0.55]{$p_{\text{RAG}}\ge\tau_{\text{RUC}}$}
    (ruc.south |- t3.north) -- (t3.north);

% RUC -> Tier 2 (zero-shot) when p_RAG < tau_RUC; loop up and over to enter t2 from the right
\draw[oarr]
    (ruc.north) -- ++(0,0.4) coordinate (rucup)
    -- node[elab,above,pos=0.5]{$p_{\text{RAG}}<\tau_{\text{RUC}}$}
    (t2.north east -| rucup) -- (t2.north east);
...
%
%  After S8 concludes, the updated weights theta_new (or theta_prev on rollback)
