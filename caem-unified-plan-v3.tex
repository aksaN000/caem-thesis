...
\node[below=0.2cm of consistency, align=center, text width=2.5cm, font=\scriptsize, fill=blue!5, inner sep=2pt, rounded corners=2pt] {$u_c = \frac{2}{M(M-1)}\sum_{i<j}\text{sim}_{ij}$};
...
\begin{equation}
u_{\text{consistency}} = \frac{2}{M(M-1)} \sum_{1 \le i < j \le M} \text{sim}(v_i, v_j)
\end{equation}
...
\textbf{Step 2 - Compute unique pairwise similarities:}
\begin{align}
...
\begin{equation}
u_{\text{consistency}} = \frac{0.95 + 0.97 + 0.93}{3} = \frac{2.85}{3} = 0.950
\end{equation}

\textbf{Interpretation:} Confidence = 0.950 (very high), chains strongly agree.
\end{examplebox}
...
\State $s_{\text{avg}} \gets \frac{2}{M(M-1)} \sum_{i<j} s_{ij}$
\State
...
\textbf{Implementation note (Fix A, Session 30):} In the actual implementation, Tier 2 and Tier 3 generation for StrategyQA uses a task-aware CoT prompt that enforces the format \texttt{``Reasoning: <explanation> Answer: yes|no''}. The chains below show the equivalent reasoning content; the stored \texttt{reasoning\_chain} in memory carries the explicit ``Reasoning:'' prefix and embedded ``Answer:'' label. SC consistency is computed by extracting and comparing the label tokens across chains using \texttt{extract\_strategyqa\_label()}.
...
\textbf{Task-aware prompting and the training target (implementation note):}
Generation prompts are task-conditioned for all four benchmarks. For open-ended benchmarks (HotpotQA, TruthfulQA), the Tier 2/3 prompt elicits free-form chain-of-thought: \texttt{``Question: \{q\}\textbackslash{}nThink step by step:''}, and \texttt{reasoning\_chain} stores the full generation. For classification benchmarks (FEVER, StrategyQA), the prompt enforces a structured rationale-then-label format:
\begin{quote}
FEVER: \texttt{Reasoning: <explanation> Answer: supports|refutes|not enough info}\\
StrategyQA: \texttt{Reasoning: <explanation> Answer: yes|no}
\end{quote}
This ensures that \texttt{reasoning\_chain} always stores a genuine reasoning trace for all four benchmarks, not a bare label token. The thesis claim of ``verified reasoning-chain supervision'' therefore holds uniformly. Label extraction at evaluation time uses regex-based label parsers (\texttt{extract\_fever\_label}, \texttt{extract\_strategyqa\_label}) rather than raw string comparison.
\end{designbox}

\textbf{Standard fine-tuning loss:}

\noindent In the implementation, the training target for each episode is the full \texttt{reasoning\_chain} field, which encodes both the reasoning trace and the answer (embedded in the chain as ``Answer: \{answer\}''). The fine-tuning objective is:

\begin{equation}
\mathcal{L}_{\text{standard}} = -\sum_{(q,c,a)} \log P_\theta(c, a | q)
\end{equation}

\noindent where $c$ is the task-conditioned reasoning trace and $a$ is the answer embedded within it. For open-ended tasks, $c$ contains sentence-level CoT steps. For classification tasks, $c$ contains a brief rationale followed by the label token. In both cases, the model is supervised on the full structured output, not on the answer string alone.
...
\textbf{Corrected condition:} The condition for $P > p$ is $\alpha > \frac{1}{2}$ (not $p > (1-\alpha)$ as stated in earlier drafts of this document --- that condition is mathematically incorrect). See the formal proof in \texttt{chapter\_4.tex} §4.9. State measured $\alpha$ explicitly in Chapter~5, confirming $\alpha > 0.5$.
\end{designbox}
...
\midrule
\multicolumn{2}{l}{\textit{Internal baselines (run on same CAEM benchmark splits):}} \\
...
\midrule
\multicolumn{2}{l}{\textit{External baselines (published numbers, cite RA-ISF: Liu et al., ACL Findings 2024, arXiv:2403.06840):}} \\
GPT-3.5 (vanilla) & Zero-shot GPT-3.5-turbo, no retrieval. HotpotQA EM: 22.1\%; StrategyQA: 65.2\%. \\
GPT-3.5 + RAG & GPT-3.5 with standard retrieval. HotpotQA EM: 32.2\%; StrategyQA: 64.7\%. \\
Self-RAG 13B & Self-RAG (Asai et al. 2023) at 13B scale. HotpotQA EM: 25.4\%; StrategyQA: 67.2\%. \\
\midrule
\textbf{CAEM (Ours)} & \textbf{Flan-T5-Large (780M), full system with all components, 3 cycles.} \\
\bottomrule
\end{tabular}
\end{table}

\begin{keyinsightbox}
\textbf{Efficiency story (PUB-01 positioning):} Self-RAG 13B uses a model 17$\times$ larger than CAEM's Flan-T5-Large backbone. If CAEM Cycle~3 approaches or matches Self-RAG's HotpotQA EM (25.4\%), report this explicitly with the parameter ratio. The framing is: \textit{verified episodic self-improvement enables a small model to approach large-model RAG performance through progressive knowledge accumulation}. This is the efficiency contribution, and it survives the ``but GPT-4 does better'' objection entirely.
\end{keyinsightbox}
