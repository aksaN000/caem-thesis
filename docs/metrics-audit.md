...
| # | Metric | Where | Semantic | Literature anchor | Validity |
|---|---|---|---|---|---|
| 15 | `_forgetting_score` (TriviaQA, 50 pairs) | `self_improvement.py:661` | in-domain generation accuracy on held-out TQA pairs; drives abort guard | GEM forgetting measure (Lopez-Paz 2017) is the conceptual cousin; ours is simpler (absolute accuracy, not ΔACC) | OK for *operational* abort gating; NOT appropriate as a reported metric — that is what `_mmlu_score` is for |
