...
| # | Element | What it must show | Source | Satisfies |
|---|---|---|---|---|
| TAB-C5-01 | **Table 5.1: Main Results — CAEM vs All Baselines** | Rows: all methods (Zero-shot, CoT, RAG, SC, Vanilla FT, Memory-only, CAEM Cycle 0, CAEM Cycle 10). Columns split into two groups: **Training benchmarks** (FEVER / TriviaQA / NQ) — CAEM rows only (baselines not evaluated on these in `run_ablation.py`); **Transfer benchmarks** (TruthfulQA / StrategyQA / ARC-Challenge / Average) — all rows including baselines. Include † for statistically significant improvements (McNemar's p < 0.05). Add a sub-row or footnote for GPT-3.5 / Self-RAG literature numbers on StrategyQA only (PUB-01c). **Note:** baselines from `run_ablation.py` use `BENCHMARK_ORDER = ["fever", "truthfulqa", "strategyqa", "arc_challenge"]` — they do NOT evaluate on TriviaQA/NQ. For those columns, baseline rows show "—". This must be stated in the table caption: "Baselines evaluated on FEVER and transfer benchmarks only; TriviaQA and NQ are training benchmarks excluded from baseline comparison to prevent circularity." **Session 35 note:** replaced old column set (HotpotQA / TruthfulQA / FEVER / StrategyQA) and "Cycle 3" with current benchmark suite and "Cycle 10". | `outputs/eval/*.json` + `ablation_results/ablation_summary.json` | C5-04, C5-05, C5-12, GEN-12 |
