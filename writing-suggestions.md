...
| IMPL-01 | **Tier 1 skips Stage 5 verification** — thesis diagram must be corrected | Correct the unified-plan TikZ diagram which shows Tier 1 flowing into Stage 5. In the thesis: "Tier 1 serves retrieved answers directly — Stage 5 verification is not re-run per query. Quality assurance is maintained by retroactive re-verification (§4.x), which re-scores all stored episodes with the cycle's improved model. This amortises verification cost over one batch per cycle rather than per query, and uses the improved model rather than the stale storage-time model — a strictly stronger freshness guarantee." Update the TikZ diagram to remove the Tier 1→Stage 5 arrow. Update Scenario 1 latency to "~150–400 ms (FAISS retrieval only)." | Session 26 / Impl Log | OPEN |
...
|---|---|---|---|
| IMPL-01 | Tier 1 skips Stage 5 verification — correct TikZ diagram + Scenario 1 latency | Ch4 §4.2 + diagram | OPEN |
